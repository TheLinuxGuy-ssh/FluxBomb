...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing FluxBomb'
    else
        figlet TBomb
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mTheLinuxGuy \e[0m"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border TheLinuxGuy
    fi
    echo " "
}
...
do
    ...
    if [ $ch -eq 1 ];then
        ...
        echo -e "\e[1;34m RUN FluxBomb Again..."
        pause
        exit
    elif [ $ch -eq 5 ];then
        banner
